...
test_that("a ridge term's edf is the eigenvalue shrinkage of its block", {
  ...
  got <- edf(built, coef = beta, hessian = H, theta = list(lambda = lam))
  # independent route: the ridge's Hessian is lambda*I, so the trace is
  # sum d_i / (d_i + lambda) over the eigenvalues of H
  d <- eigen(H, symmetric = TRUE, only.values = TRUE)$values
  expect_equal(got, sum(d / (d + lam)), tolerance = 1e-12)

  # the limits: no penalty recovers the count, a hard one removes it
  expect_equal(edf(built, beta, H, list(lambda = 1e-12)), 3, tolerance = 1e-6)
  expect_lt(edf(built, beta, H, list(lambda = 1e8)), 1e-4)
})
...
test_that("edf counts a partially penalized term parameter by parameter", {
  ...
  expect_equal(edf(br, coef = c(0.5, 2, 6), hessian = H,
                   theta = list(lambda = 1e-12)), 3, tolerance = 1e-6)
  expect_equal(edf(br, coef = c(0.5, 2, 6), hessian = H,
                   theta = list(lambda = 1e16)), 2, tolerance = 1e-6)
...
test_that("edf refuses what it cannot compute and says what is missing", {
  ...
  expect_error(edf(br, coef = 1, hessian = diag(2), theta = list(lambda = 1)),
               "length 2")
  expect_error(edf(br, coef = c(1, 2), hessian = diag(3),
                   theta = list(lambda = 1)), "2 x 2")
