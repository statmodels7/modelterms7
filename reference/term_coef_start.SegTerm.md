...
here rather than neutral – a discontinuous term reads its break-point
off \\-g_k/\delta_k\\, which at zero is the same clamped position for
every one of them, and a continuous term's Jacobian column vanishes – so
a fitting layer that starts every coefficient at zero has to be told
otherwise.
