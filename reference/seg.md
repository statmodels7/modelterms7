...
a break-point, and the reported position says so.

The objective has local optima in the break-points, so a run should be
started from several positions;
[`multistart`](https://statmodels7.github.io/optimizers7/reference/multistart.html)
is the toolkit's version of the bootstrap restarting the segmented
literature uses. The step also has to be damped, and how much is a
property of the construction: on a joint jump and change of slope at \\x
= 5\\ in 500 observations, a step of 0.2 reaches 5.01 while one of 0.4
leaves the data and stops at the lower limit with a residual sum of
squares thirty-seven times larger. The continuous case is the least
demanding of the three, the discontinuous one the most.
