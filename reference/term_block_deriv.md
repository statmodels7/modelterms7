...
and the columns and answers per coefficient, which is what the gradient
of a marginal criterion needs; this one contracts over the coefficients
and answers per entry of the block, which is what its HESSIAN needs –
there \\\partial K/\partial\beta\\ is required in the direction the mode
moves rather than traced.
...
q\_{p_1p_2}(i)\\(Z\_{p_2}v\_{p_2})\[i\],\$\$ which is the second
derivative of \\f\\ and not the third.
