...
past the data is the score: it has zero conditional mean by
...
known and the filter must be run instead of continued, so it is rejected
with the rows named. A group the fit never saw is rejected for the same
