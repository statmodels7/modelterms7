...
as a held value would be, and nothing else is applied to them – the
value that empties the block does not cap them and the depth of the path
does not reach them, the caller having said which values to visit.
...
penalty, so the direction is settled where the path is built and not
here.
